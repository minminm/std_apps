...
fn test_os_string_with_capacity() {
    ...
    assert_eq!(0, os_string.into_encoded_bytes().capacity());
                  
    let os_string = OsString::with_capacity(10);
    assert_eq!(10, os_string.into_encoded_bytes().capacity());

    let mut os_string = OsString::with_capacity(0);
    os_string.push("abc");
    assert!(os_string.into_encoded_bytes().capacity() >= 3);
}

fn test_os_string_clear() {
    let mut os_string = OsString::from("abc");
    assert_eq!(3, os_string.as_encoded_bytes().len());

    os_string.clear();
    assert_eq!(&os_string, "");
    assert_eq!(0, os_string.as_encoded_bytes().len());
}
...
fn main() {
    test_os_string_with_capacity();

    test_os_string_clear();

    test_os_string_capacity();

    test_os_string_reserve();

    test_os_string_reserve_exact();

    test_os_string_join();

    test_os_string_default();

    test_os_str_is_empty();
    ...
    // panic
    #[cfg(windows)]
    slice_between_surrogates();

    // panic
    #[cfg(windows)]    
    slice_surrogate_edge();
