...
fn copy_file_does_not_exist() {
    let from: &Path = Path::new("test/nonexistent-bogus-path");
...
fn open_flavors() {
    ...
    // #[cfg(all(unix, not(target_os = "vxworks")))]
    let invalid_options = "Invalid input parameter";
...
fn main() {
    ...
    // not support yet (symlink and hardlink)
    ...
    // not support yet (chmod)
    // chmod_works();
    // fchmod_works();

    // not support fully (fake flush)
    sync_doesnt_kill_anything();

    // not support(sync_all can't flush metadata)
    // truncate_works();
    ...
    test_eq_direntry_metadata();

    test_read_dir_infinite_loop();

    // not support (rename, both in ramfs and 9p)
    // rename_directory();
