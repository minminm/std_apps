...
pub fn test_push() {
    ...
    if cfg!(unix) || cfg!(target_os = "ruxos") || cfg!(all(target_env = "sgx", target_vendor = "fortanix")) {
        tp!("", "foo", "foo");
        tp!("foo", "bar", "foo/bar");
        tp!("foo/", "bar", "foo/bar");
        tp!("foo//", "bar", "foo//bar");
...
fn bench_path_cmp_fast_path_buf_sort() {
    let prefix = "my/home";
    let mut paths: Vec<_> =
    ...
    // });
    for _ in 0..1000 { 
        std::hint::black_box(paths.as_mut_slice()).sort_unstable();
    }
}

// #[bench]
// #[cfg_attr(miri, ignore)] // Miri isn't fast...
fn bench_path_cmp_fast_path_long() {
...
// #[bench]
// #[cfg_attr(miri, ignore)] // Miri isn't fast...
fn bench_path_cmp_fast_path_short() {
    let prefix = "my/home";
    let paths: Vec<_> =
        (0..1000).map(|num| PathBuf::from(prefix).join(format!("file {num}.rs"))).collect();
...
fn bench_path_hashset() {
    ...
    for _ in 0..1000 { 
        set.remove(paths[500].as_path());
        set.insert(std::hint::black_box(paths[500].as_path()));
    }
}

// #[bench]
// #[cfg_attr(miri, ignore)] // Miri isn't fast...
fn bench_path_hashset_miss() {
    let prefix = "/my/home/is/my/castle/and/my/castle/has/a/rusty/workbench/";
    let paths: Vec<_> =
        (0..1000).map(|num| PathBuf::from(prefix).join(format!("file {num}.rs"))).collect();
    ...
    for _ in 0..1000 { 
        set.remove(std::hint::black_box(probe.as_path()));
    }
}

// #[bench]
fn bench_hash_path_short() {
    use std::hash::{Hash, Hasher};
    ...
    for _ in 0..1000 { 
        std::hint::black_box(path).hash(&mut hasher);
    }
...
fn main() {
    ...
    bench_path_cmp_fast_path_buf_sort();

    bench_path_cmp_fast_path_long();

    bench_path_cmp_fast_path_short();

    bench_path_hashset();

    bench_path_hashset_miss();
